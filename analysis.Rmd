...
meta_csv <- read.csv('../input/TIALP-B1/Metadatafile TopoChip_TiALP_array1_allunits_pruned_fixedmetaproblem.csv')
...
#pred_var <- "ALPCytoplasm_Intensity_IntegratedIntensity_ALP4Corr"
pred_var <- "ALPCytoplasm_Intensity_MeanIntensity_ALP4Corr"

D <- readRDS('../results/HEAD/8658d87a/Dbx.rda')

h <- hash()
h["Cells_AreaShape_Area"] <- "Area"
h["Cells_AreaShape_MajorAxisLength"] <- "MajorAx"                
h["Cells_AreaShape_MinorAxisLength"] <- "MinorAx"               
h["Cells_AreaShape_Solidity"] <- "Solidity"    
h["Cells_Intensity_IntegratedIntensity_Actin4Corr"] <- "IntegIntActin"
h["Cells_Intensity_MaxIntensity_Actin4Corr"] <-  "MaxIntActin"  
h["Cells_Intensity_StdIntensity_Actin4Corr"] <-  "StdIntActin"        
h["Cells_Neighbors_PercentTouching_Expanded"] <- "PCtouch"
h["Nuclei_Number_Object_Number"] <- "Count" # Not exactly cell count
h["Cells_AreaShape_Zernike_2_0"] <- "Zer2_0"
h["Cells_AreaShape_Zernike_2_2"] <- "Zer2_2"
h["Cells_AreaShape_Extent"] <- "Extent"
h["Cells_AreaShape_Perimeter"] <- "Peri"
h["Cells_Intensity_MassDisplacement_Actin4Corr"] <- "MassDispActin"
h["Cells_Neighbors_SecondClosestDistance_Expanded"] <- "Neigbor2ndDist"
h["Cells_RadialDistribution_FracAtD_Actin4Corr_1of4"] <- "RadDirActin14"
h["Cells_RadialDistribution_FracAtD_Actin4Corr_4of4"] <- "RadDirActin44"

shorten_names <- function(df, h){
  for(k in names(as.list(h)))
    names(df)[names(df)==k] <- h[[k]]
  df
}
#D <- shorten_names(D,h)
...
# color the nodes
NCOLORS <- 10
#leafcols_pos <- colorRampPalette(brewer.pal(9, "Blues"))(NCOLORS)
#leafcols_neg <- colorRampPalette(brewer.pal(9, "Oranges"))(NCOLORS)
#get_leafcol <- function(x, s) if (s < 0) leafcols_neg[[x]] else leafcols_pos[[x]]

leafcols <- colorRampPalette(brewer.pal(9, "PRGn"))(2*NCOLORS+1)
get_leafcol <- function(x, s) leafcols[[1+NCOLORS+s*x]]
rescale_col <- function(x, step=4) step + ceiling((NCOLORS-step) * (abs(x)-min(abs(vmean))) / (max(abs(vmean)-min(abs(vmean)))))
...
remap_names <- function(n) {
    if (is.leaf(n))      
        attr(n, "label") <- paste(strsplit(attr(n, "label"), ':')[[1]][1], h[[strsplit(attr(n, "label"), ':')[[1]][2]]], sep=':')
    n  
}

dendroC <- dendrapply(dendro, colLab)
dendroR <- dendrapply(dendroC, remap_names)

# plot dendrogram
#pdf('dendro.pdf', width = 8, height = 5)
pdf(sprintf('dendro_%s.pdf', pred_var), width = 12, height = 8)
par(mar = c(5, 15, 5, 25))
plot(dendroC, horiz = TRUE, type = "triangle", center = T, main=pred_var)
dev.off()

# #pdf('dendro_abbrev.pdf', width = 6, height = 5)
# pdf(sprintf('dendro_abbrev_%s.pdf', pred_var), width = 6, height = 5)
# par(mar = c(5, 15, 5, 25))
# plot(dendroR, horiz = TRUE, type = "triangle", center = T, main=pred_var)
# dev.off()


```

```{r perform_sem, eval=FALSE}
...
# Plot pairwise scatterplot of features
Dsub_r <- shorten_names(as.data.frame(Dsub), h)
Dsub_mr <- as.data.frame(Dsub_m)
names(Dsub_mr) <- names(Dsub_r)

pdf('pairwise_hexplot_feats.pdf', width = 25, height = 25)
hexplom(Dsub_mr, xbins=20, upper.panel = panel.hexboxplot)
dev.off()


# Build SEM model
feats <- setdiff(names(vmean) , c('Cells_RadialDistribution_FracAtD_Actin4Corr_4of4', 
                                  'Cells_AreaShape_MinorAxisLength',
                                  'Cells_AreaShape_Perimeter'))
Dsub.cov <- cor(Dsub[,feats])
...
fs <- fscores(Dsub.sem)
Dsub_f <- as.matrix(Dsub[,feats]) %*% fs
pdf('pairwise_hexplot_factors.pdf', width = 8, height = 8)
hexplom(Dsub_f, xbins=20, upper.panel = panel.hexboxplot)
dev.off()
cor(Dsub_f)

```

```{r Compare_var_groups_pred_ALP, eval=FALSE}
# Compare variable groups for predicting ALP
varsel = list()
varsel$only_actin_vars <- c('Cells_Intensity_IntegratedIntensity_Actin4Corr',
          'Cells_Intensity_StdIntensity_Actin4Corr',
          'Cells_RadialDistribution_FracAtD_Actin4Corr_1of4',
          'Cells_RadialDistribution_FracAtD_Actin4Corr_4of4')
varsel$no_actin_vars <- setdiff(names(vmean), varsel$only_actin_vars)
varsel$all_vars <- names(vmean)
for (s in names(varsel)) {
  X <- D[,varsel[[s]]]
  y <- D[,pred_var]
  yt <- predict(lm(y~as.matrix(X)))
  plot(hexbin(y, yt), main=sprintf("%s: Cor=%0.2f", s, cor(yt, y)))
}
