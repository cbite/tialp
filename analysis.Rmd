...
corD <- abs(corDs)
vmean <- corD[,pred_var]
vmeans <- corDs[,pred_var]
cor_thresh <- 0.3
feat_flag <- vmean > cor_thresh
corD <- corD[feat_flag,feat_flag]
vmean <- vmean[feat_flag] 
tree <- hclust(as.dist(1 - abs(corD)))
tree.labels <- tree$labels
tree$labels <- unlist(lapply(tree$labels, function(x) sprintf("%0.2f:%s", vmeans[[as.character(x)]], 
    as.character(x))))
library(colorRamps)
library(RColorBrewer)
NCOLORS <- 10
leafcols <- colorRampPalette(brewer.pal(9, "Blues"))(NCOLORS)
#lab.col <- lapply(tree.labels, function(x) leafcols[[ceiling(NCOLORS * (0.3+0.7*(vmean[[as.character(x)]])))]])
lab.col <- lapply(tree.labels, function(x) leafcols[[ceiling(NCOLORS * ((vmean[[as.character(x)]])))]])
...
colLab <- function(n) {
    if (is.leaf(n)) {
        a <- attributes(n)
        attr(n, "nodePar") <- c(a$nodePar, list(lab.col = lab.col[[attr(n, "label")]]))
    }
    n
}
